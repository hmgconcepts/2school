-- ============================================================================
-- HMG SCHOOL CONNECT v11 — COMPLETE SELF-CONTAINED DATABASE SCHEMA
-- ============================================================================
...
set school_id = coalesce(school_settings.school_id, (select id from public.schools order by created_at limit 1)),
    school_name = coalesce(nullif(school_settings.school_name,''), (select name from public.schools order by created_at limit 1), 'My School'),
    short_name = coalesce(nullif(school_settings.short_name,''), (select short_name from public.schools order by created_at limit 1), 'SCH'),
    admission_acronym = coalesce(nullif(school_settings.admission_acronym,''), nullif(school_settings.short_name,''), 'SCH'),
    admission_prefix = coalesce(nullif(school_settings.admission_prefix,''), nullif(school_settings.admission_acronym,''), 'SCH'),
    staff_prefix = coalesce(nullif(school_settings.staff_prefix,''), nullif(school_settings.short_name,''), 'SCH')
where id = 1;
...
-- School Connect — Database Schema (Gen v11)
-- =====================================================================
...
-- ==========================================
-- ENTERPRISE TABLES & ENHANCEMENTS
-- ==========================================
